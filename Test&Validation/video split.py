...
def record_continuous_video(output_path, duration=None):
    ...
    return video_name  # Return the path to the recorded video


def split_video(video_path, output_dir, clip_duration):
    cap = cv2.VideoCapture(video_path)
    frame_rate = int(cap.get(cv2.CAP_PROP_FPS))
    frames_per_clip = frame_rate * clip_duration
...
if __name__ == "__main__":
    ...
    # Record a continuous video for a specified duration (e.g., 60 seconds)
    try:
        continuous_video_path = record_continuous_video(new_dir, duration=100)  # Adjust duration as needed
        ...
        # Split continuous video into clips of different durations
        split_video(continuous_video_path, one_sec_dir, clip_duration=1)
        split_video(continuous_video_path, two_sec_dir, clip_duration=2)
        split_video(continuous_video_path, five_sec_dir, clip_duration=5)
        split_video(continuous_video_path, ten_sec_dir, clip_duration=10)
